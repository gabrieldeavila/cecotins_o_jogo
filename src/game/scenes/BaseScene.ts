...
export abstract class BaseScene extends Scene {
    ...
    create() {
        ...
        // 1. Setup básico e Mapa (sem criar colisões com player ainda)
        this.setupMap(config);

        // 2. Setup de Controles
        this.setupControls();

        // 3. Criar o Player (IMPORTANTE: deve vir antes dos colliders/overlap)
        this.setupPlayer();

        // 4. Agora sim, setup de física e colisões que dependem do player
        this.setupPhysics();
        this.createCollectibles(this.map);

        // 5. Restante dos sistemas
    ...
    private setupMap(config: LevelConfig) {
        ...
        const tilesetBlue = this.map.addTilesetImage(
            "Blue back",
            config.tileSetBackground,
        );
    ...
    protected handleMovement() {
        ...
        // Corrigido: Agora verifica a seta para cima (cursors.up) e o Enter
    ...
    private createCollectibles(map: Phaser.Tilemaps.Tilemap) {
        ...
        // Este era o ponto do erro: agora garantimos que this.player existe antes de criar o overlap
